...
update_feeds() {
    ...
    if ! grep -q "small-package" "$FEEDS_PATH"; then
        ...
        echo "src-git small8 https://github.com/nywangyu/OpenWrt-Packages;24.10" >>"$FEEDS_PATH"
    fi

    if ! grep -q "openwrt-nikki" "$FEEDS_PATH"; then
        [ -z "$(tail -c 1 "$FEEDS_PATH")" ] || echo "" >>"$FEEDS_PATH"
        echo "src-git nikki https://github.com/nikkinikki-org/OpenWrt-nikki;main" >>"$FEEDS_PATH"
    fi
